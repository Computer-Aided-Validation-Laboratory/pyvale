...
working directory so that ``pyvale-output`` scratch files never land in the
repository. A summary is printed at the end and the exit code is non-zero if
any example failed.
"""
...
def main() -> None:
    ...
    if failed:
        ...
        for label, message in failed:
            print(f"  FAIL  {label} ({message})")
    print("=" * 70)
    print(f"Session working directories kept for inspection: {session_dir}")
