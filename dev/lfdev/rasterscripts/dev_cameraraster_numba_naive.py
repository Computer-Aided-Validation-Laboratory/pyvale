...
@numba.jit(nopython=True)
def crop_loop(elem_world_coords: np.ndarray,
              world_to_cam_mat: np.ndarray,
              image_dims: np.ndarray,
              image_dist: float,
              num_pixels: np.ndarray
    ...
    xx: int = 0
    yy: int = 1
    elem_count = elem_world_coords.shape[2]
    nodes_per_elem = elem_world_coords.shape[0]

    crop_mask = np.full(elem_count,True)
    x_min = np.empty(elem_count,dtype=np.float64)
    x_max = np.empty(elem_count,dtype=np.float64)
    y_min = np.empty(elem_count,dtype=np.float64)
    y_max = np.empty(elem_count,dtype=np.float64)
    nodes_raster = np.empty((4,nodes_per_elem,elem_count),dtype=np.float64)
    ...
    for ee in range(elem_count):
        ...
        # shape=(coords[X,Y,Z,W],nodes_per_elem)
        nodes_raster[:,:,ee] = world_to_raster_coords(nodes_world,
                                                      world_to_cam_mat,
                                                      image_dims,
                                                      image_dist,
                                                      num_pixels)
        ...
        x_min[ee] = np.min(nodes_raster[xx,:,ee])
        x_max[ee] = np.max(nodes_raster[xx,:,ee])
        y_min[ee] = np.min(nodes_raster[yy,:,ee])
        y_max[ee] = np.max(nodes_raster[yy,:,ee])

        # Cropping: if the element is outside the image we don't process it
        if ((x_min[ee] > num_pixels[xx]-1) or (x_max[ee] < 0)
            or (y_min[ee] > num_pixels[yy]-1) or (y_max[ee] < 0)):
            crop_mask[ee] = False

    return (crop_mask,x_min,x_max,y_min,y_max,nodes_raster)


#RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
def raster_loop(elem_world_coords: np.ndarray,
                field_to_render: np.ndarray,
                world_to_cam_mat: np.ndarray,
                image_dims: np.ndarray,
                image_dist: float,
                num_pixels: np.ndarray,
                sub_samp: int) -> tuple[np.ndarray,np.ndarray]:

    xx: int = 0
    yy: int = 1
    zz: int = 2

    depth_buffer = 1e6*np.ones(sub_samp*num_pixels).T
    image_buffer = np.full(sub_samp*num_pixels,0.0).T


    (crop_mask,x_min,x_max,y_min,y_max,nodes_raster) = crop_loop(
        elem_world_coords,
        world_to_cam_mat,
        image_dims,
        image_dist,
    ...
    x_min = x_min[crop_mask]
    x_max = x_max[crop_mask]
    y_min = y_min[crop_mask]
    y_max = y_max[crop_mask]
    nodes_raster = nodes_raster[:,:,crop_mask]
    field_to_render = field_to_render[:,crop_mask]
    elems_after_crop = nodes_raster.shape[-1]


    for ee in range(elems_after_crop):
        xi_min = np.max((0,np.floor(x_min[ee]))).astype(np.int64)
        xi_max = np.min((num_pixels[xx]-1, np.ceil(x_max[ee]))).astype(np.int64)
        yi_min = np.max((0,np.floor(y_min[ee]))).astype(np.int64)
        yi_max = np.min((num_pixels[yy]-1, np.ceil(y_max[ee]))).astype(np.int64)

        # Pre-divide the z coord for interpolation
        nodes_raster[zz,:,ee] = 1 / nodes_raster[zz,:,ee]
        ...
        nodes_field = nodes_field * nodes_raster[zz,:,ee]

        elem_area = edge_function(nodes_raster[:,0,ee],
                                  nodes_raster[:,1,ee],
                                  nodes_raster[:,2,ee])

        bound_coords_x = (np.arange(xi_min,xi_max,1/sub_samp)
                        + 1/(2*sub_samp))
        bound_coords_y = (np.arange(yi_min,yi_max,1/sub_samp)
                        + 1/(2*sub_samp))

        bound_inds_x = np.arange(sub_samp*xi_min,
                                 sub_samp*xi_max)
        bound_inds_y = np.arange(sub_samp*yi_min,
                                 sub_samp*yi_max)
        ...
        for jj in range(bound_coords_y.size):
            for ii in range(bound_coords_x.size):
                ...
                weights = np.empty(3)
                weights[0] = edge_function(nodes_raster[:,1,ee],
                                           nodes_raster[:,2,ee],
                                           px_coord)
                weights[1] = edge_function(nodes_raster[:,2,ee],
                                           nodes_raster[:,0,ee],
                                           px_coord)
                weights[2] = edge_function(nodes_raster[:,0,ee],
                                           nodes_raster[:,1,ee],
                                           px_coord)

                if np.all(weights > 0.0):
                    weights = weights / elem_area
                    px_coord_z = 1/(np.sum(weights[:]*nodes_raster[zz,:,ee]))
...
def main() -> None:
    ...
    print(80*"=")
    for ll in range(num_loops):
        loop_start = time.perf_counter()
        print(f"Running raster loop {ll+1} of {num_loops}")
        (image_buffer,depth_buffer) = raster_loop(elem_world_coords,
                                                  field_to_render,
    ...
    print()
    for ll in range(num_loops):
        print(f"Loop {ll+1} time = {loop_times[ll]} seconds")
    print(80*"=")
