"""Run two-phase univariate-Gaussian VFM identification for a notched weld.

The first phase identifies homogeneous yield strength and hardening modulus.
The second phase grows circular Gaussian basis functions in the yield-strength
field from the combined equilibrium-gap indicator. The more extensive
centre-screening investigation is archived separately in ``dev/vfm/rob-scripts``.
"""
...
FORCE_SLICES = 63
SBVF_MESH_SIZE = np.asarray((10, 5), dtype=np.uint32)
...
PHASE_1_MAX_EVALUATIONS = 5000  # eval_count ~ 1 + (iter_count * ((2*dof_count) + 1)) i.e. for 6 dof, 1 + (iter_count * 13)
PARALLEL_WORKERS = 12
# Select the backend used when running this file directly from VS Code.
# Use "cython" for the local compiled linear-hardening accelerator or
# "numpy" to use Pyvale's reference implementation.
...
def main() -> None:
    ...
    # Define phase 0: homogeneous yield-strength and hardening identification
    ...
    # Define phase 1 metrics and objective
    ...
    equilibrium_gap_metrics_phase_1 = [
        EquilibriumGapMetric(window_size=window) for window in EGI_WINDOWS
    ]

    # Define EGI window weights for phase 1
    egi_window_weights = [window[0] for window in EGI_WINDOWS]
    yield_strength_basis = SpatialParameterisationBasisFunction(x=x, y=y)
    ...
    # Save results
    output_dir = args.output_root / experiment_data_file.parent.name / "univariate_gaussian"
    ...
    summary = {
        ...
        "phases": ["homogeneous", "univariate_gaussian"],
    }
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max-evaluations",
        type=int,
        default=PHASE_1_MAX_EVALUATIONS,
        help="Maximum pattern-search evaluations in phase 1.",
    )
    ...
    parser.add_argument(
        ...
        help="Stress-reconstruction backend; overrides STRESS_BACKEND.",
    )
    parser.add_argument(
        "--no-progress",
        action="store_false",
