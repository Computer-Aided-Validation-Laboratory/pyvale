...
POSTPROCESSING_OUTPUT_DIR = (
    DATASET_ROOT
    / "identification"
    / "notched-weld-vertical-slice_postprocessing_output"
)

# Compare identified maps against the known FE material maps.
KNOWN_PARAMETER_MAPS = (
    EXPERIMENT_DATA_PATH / "known_parameter_maps.npz"
)

# The weld regions are arranged along x:
# BM-left: 12.5–35 mm, WM: 35–40 mm, BM-right: 40–62.5 mm.
DIAGNOSTIC_AXIS = "x"
DIAGNOSTIC_SLICES = 3
...
def main() -> None:
    ...
    centre_row = experiment_data.strain.shape[2] // 2
    # Approximate material-band midpoints within the central ligament.
    target_x_coordinates = [32.5, 37.5, 42.5]  # BM-left, WM, BM-right
    x_grid = experiment_data.specimen_geometry.x[centre_row, :]
    point_rows = [centre_row] * len(target_x_coordinates)
    point_columns = [
        int(np.argmin(np.abs(x_grid - target_x)))
        for target_x in target_x_coordinates
    ]
