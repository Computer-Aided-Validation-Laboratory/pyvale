...
def focal_length_from_resolution(
    ...
    return working_dist * pixel_size[0] / resolution
...
def blender_field_of_view(camera: Camera) -> tuple[float, float]:
    ...
    field_y = camera.pixels_count[1] / camera.pixels_count[0] * field_x
...
def blender_mm_per_pixel(camera: Camera) -> float:
    ...
    return blender_field_of_view(camera)[0] / camera.pixels_count[0]
