...
namespace lm {
    ...
    std::vector<std::vector<double>> hessian(6, std::vector<double>(6, 0.0));
    std::vector<std::vector<double>> inverse(6, std::vector<double>(6, 0.0));
    ...
    void deformed_subset_constants(std::vector<double> &subset_def, int n){
    ...
    void calculate_gradient(
        ...
        for (int i = 0; i < n; i++){
            ...
            // ssd
            double prefactor = (subset_ref[i] - subset_def[i]);
            ...
            for (int row = 0; row < 6; row++){
                for (int col = 0; col < 6; col++){
                    hessian[row][col] += dfdp[row] * dfdp[col];
                }
            }
        }
    }
    ...
    void calculate_hessian(){
        ...
        for (int row = 0; row < 6; row++){
            for (int col = 0; col < 6; col++){
                ...
                // znssd
                // hessian[row][col] =  const_term * dfdp[row] * dfdp[col];
                // hessian[count] =  const_term * dfdp[row] * dfdp[col];
                
                // ssd
                hessian[row][col] =  dfdp[row] * dfdp[col];
                // hessian[count] =  dfdp[row] * dfdp[col];
                
                if (row == col){
                    // hessian[row][col] += lambda * hessian[row][col];
    ...
    void calculate_deltap() {
        ...
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 6; ++j) {
                ...
                // dp[i] +=  1.0 * inverse[i][j] * gradient[j];
            }
        }
    ...
    bool invertMatrix(const std::vector<std::vector<double>>& matrix, std::vector<std::vector<double>>& inverse) {
        int n = 6;
        std::cout << "OFEUNAOIUNEAOINFOIA" << std::endl;
