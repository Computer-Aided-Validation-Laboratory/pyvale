...
def plot_point_sensors_on_sim(sensor_array: SensorsPoint,
    ...
    if type(comp_key) == str:
        comp_key = [comp_key]
        if len(comp_key) < len(sensor_arraylist):
            comp_key = comp_key*len(sensor_arraylist)

    if len(sensor_arraylist) < len(comp_key):
        if len(sensor_arraylist) == 1:
