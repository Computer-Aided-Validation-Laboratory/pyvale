...
//     color color_from_emission = hit.material_ptr->emitted(ray, hit, hit.u, hit.v, hit.position);

//     if (!hit.material_ptr->scatter(ray, hit, srec))
//         return color_from_emission;

//     if (srec.skip_pdf) {
//         return srec.attenuation * scene.cam.ray_color(srec.skip_pdf_ray, depth-1, scene.hittable_list, scene.importance_sampled_list);
//     }
// }
...
PYBIND11_MODULE(pyray, m) {
    ...
    py::class_<Lambertian, Material, shared_ptr<Lambertian>>(m, "Lambertian")
            .def(py::init<color>())
...
*/
    ...
    py::class_<Hittable_list, Hittable, shared_ptr<Hittable_list>>(m, "Hittable_list")
    .def(py::init<>())
