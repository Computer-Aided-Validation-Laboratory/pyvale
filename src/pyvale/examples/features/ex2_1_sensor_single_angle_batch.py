...
def main() -> None:
    ...
    sensor_data_norot = pyv.SensorData(positions=sensor_positions,
                                          sample_times=sample_times)


    sensor_data_rot = pyv.SensorData(positions=sensor_positions,
                                  sample_times=sample_times,
                                  angles=sensor_angles)

    #---------------------------------------------------------------------------
    disp_sensors_norot = pyv.SensorArrayPoint(sensor_data_norot,
                                                disp_field,
                                                descriptor)


    disp_sensors_rot = pyv.SensorArrayPoint(sensor_data_rot,
                                               disp_field,
                                               descriptor)
    ...
    pyv.print_measurements(disp_sensors_norot,
    ...
    pyv.print_measurements(disp_sensors_rot,
                              (sens_to_print-1,sens_to_print),
                              (0,1),
    ...
    if plot_field == "disp_x":
        pv_plot = pyv.plot_point_sensors_on_sim(disp_sensors_rot,"disp_x")
        pv_plot.show(cpos="xy")
    elif plot_field == "disp_y":
        pv_plot = pyv.plot_point_sensors_on_sim(disp_sensors_rot,"disp_y")
        pv_plot.show(cpos="xy")

    (fig,ax) = pyv.plot_time_traces(disp_sensors_norot,plot_field)
    ax.set_title("No rotation")
    (fig,ax) = pyv.plot_time_traces(disp_sensors_rot,plot_field)
