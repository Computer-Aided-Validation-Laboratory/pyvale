...
class Quadrature2D(IIntegratorSpatial):
    ...
    """
    __slots__ = ("field","area","gauss_pts_num","gauss_pt_offsets"
                 ,"gauss_weight_func","gauss_pts","averages","sens_data")
    ...
    def __init__(self,
        ...
        self.area = self.sens_data.spatial_dims[0] * \
            self.sens_data.spatial_dims[1]

        self.gauss_pts_num = gauss_pt_offsets.shape[0]
        self.gauss_pt_offsets = gauss_pt_offsets
        self.gauss_weight_func = gauss_weight_func

        self.gauss_pts = create_int_pt_array(self.sens_data,
                                              self.gauss_pt_offsets)
        self.averages = None
    ...
    def calc_integrals(self, sens_data: SensorData | None = None) -> np.ndarray:
        ...
        """
        self.averages = self.calc_averages(sens_data)
        return self.area*self.get_averages()
    ...
    def calc_averages(self, sens_data: SensorData | None = None) -> np.ndarray:
        ...
        gauss_vals = self.field.sample_field(self.gauss_pts,
                                              self.sens_data.sample_times,
                                              self.sens_data.angles)

        meas_shape = (self.sens_data.positions.shape[0],
                      gauss_vals.shape[1],
                      gauss_vals.shape[2])

        # shape=(n_gauss_pts,n_sens,n_comps,n_timesteps)
        gauss_vals = gauss_vals.reshape((self.gauss_pts_num,)+meas_shape,
                                         order='F')

        # shape=(n_gauss_pts,n_sens,n_comps,n_timesteps)
        gauss_weights = self.gauss_weight_func(meas_shape)
        ...
        self.averages = 1/4 * np.sum(gauss_weights*gauss_vals,axis=0)
