...
class Camera:
    ...
    __slots__ = ['image_width', 'image_height', 'camera_center', 'point_camera_target', 'angle_vertical_view', 'vector_view_up','matrix_camera_to_world',
                 'matrix_world_to_camera', 'matrix_rotation', 'matrix_pixel_spacing', 'viewport_upper_left', 'pixel_00_center', 'focal_length', 'angle_cone',
                 'matrix_defocus_disc', 'viewport_bottom_right', 'viewport_width', 'viewport_height']
    ...
    def __init__(self, image_width: int, image_height: int, camera_center: np.ndarray = np.array([0.0, 0.0, 0.0]), point_camera_target: np.ndarray = np.array([0, 0, -1]),
        ...
        self.focal_length: float = focal_length # If left at 0.0, this is focal_length (where parallel rays converge); else this acts as a focus distance (distance from lens to the subject plane that is in sharp focus)
        self.angle_cone: float = radians(angle_cone) # 0 => No blur?
        self.matrix_defocus_disc: np.ndarray = np.zeros((2, 3), dtype=np.float64)
        # Basis matrices
        self.matrix_camera_to_world: np.ndarray = np.zeros((4, 4), dtype=np.float64)
        self.matrix_world_to_camera: np.ndarray = np.zeros((4, 4), dtype=np.float64)
        self.matrix_rotation: np.ndarray = np.zeros((3, 3), dtype=np.float64) # = matrix_camera_to_world[:3, :3]
        # Viewport parameters
        self.matrix_pixel_spacing: np.ndarray = np.zeros((2, 3), dtype=np.float64)
        self.viewport_upper_left: np.ndarray = np.zeros((3), dtype=np.float64)
        self.pixel_00_center: np.ndarray = np.zeros((3), dtype=np.float64)
        # We do not need to store the below for ray tracing per se, but they are here for convenience and as helpers for SceneVisualiser
        self.viewport_bottom_right: np.ndarray = np.zeros((3), dtype=np.float64)
        self.viewport_width: float = 0.0
    ...
    def _create_viewport(self,
        ...
        self.viewport_height = 2 * h_temp * focal_length  # world units (arbitrary)
        self.viewport_width = self.viewport_height * (self.image_width / self.image_height)  # world units (arbitrary)
        # Viewport basis vectors
        vector_viewport_x_axis = self.viewport_width * basis_vector_right  # Vu
        vector_viewport_y_axis = (-self.viewport_height) * basis_vector_up  # Vw
        ...
        # We push viewport away by focus distance, so the rays align with the intended focus plane
        ...
        self.viewport_bottom_right = np.array([self.viewport_upper_left[0] + self.viewport_width, self.viewport_upper_left[1] - self.viewport_height, self.viewport_upper_left[2]])
    ...
    def _find_defocus_basis_vectors(self, basis_vector_right, basis_vector_up):
        ...
        # These should not be unit vectors beause this defines the virtual aperture size, so we want to keep them scaled by radius_defocus_disc 
    ...
    def print_view_dims(self) -> None:
        ...
        print(f"Viewport size [world units]:\n \twidth: {np.round(self.viewport_width,3)}\n \theight: {np.round(self.viewport_height,)}")
        print(f"Viewport coordinates[world units]:\n \ttop left corner: {np.round(self.viewport_upper_left,3)}\n \tbottom right corner: {np.round(self.viewport_bottom_right,3)}")
