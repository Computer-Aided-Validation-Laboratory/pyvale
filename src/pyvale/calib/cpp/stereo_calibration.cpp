...
void stereo_calibration(const py::array_t<double> &dots_cam0_arr,
                        const py::array_t<double> &dots_cam1_arr,
                        const py::array_t<double> &grid_arr,
                        const std::vector<int> &lengths,
                        const int px_hori, const int px_vert, const int num_img_pairs){

    // get raw pointers
    double* grid = static_cast<double*>(grid_arr.request().ptr);
    double* dots_cam0 = static_cast<double*>(dots_cam0_arr.request().ptr);
    double* dots_cam1 = static_cast<double*>(dots_cam1_arr.request().ptr);


    //check lengths
    for (int i = 0; i < lengths.size(); i++){
        std::cout << "num points in img " << i << ": " << lengths[i] << std::endl;
    }

    std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
    std::cout << num_img_pairs << std::endl;
    std::cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
