...
bool* propagate_roi(
    ...
    std::vector<bool> roi_vec(conf.px_hori * conf.px_vert, false);
    ...
    for (int i = 0; i < results_def.conv.size(); i++)
    {
        ...
        for (int dy = -half; dy <= half; dy++)
        {
            for (int dx = -half; dx <= half; dx++)
            {
                ...
                roi_vec[py*conf.px_hori+px] = true;
            }
        }
    }
    ...
    // Copy vector → raw array
    for (int i = 0; i < conf.px_hori * conf.px_vert; i++)
    {
        roi_out[i] = roi_vec[i];
    }
    ...
    if (debug)
    {
        for (int y = 0; y < conf.px_vert; y++)
        {
            ...
            {
                std::cout << x << " " << y << " "
                          << roi_out[y * conf.px_hori + x] << std::endl;
            }
        }
    }
