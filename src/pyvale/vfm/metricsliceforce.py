...
class SliceLocalForceReconstructionProblem:
...
class SliceWiseForceReconstructionMetric(IMetric):
    ...
    def __post_init__(self) -> None:
        ...
        self._slice_force_point_weights = tuple(operator[1] for operator in slice_force_operators)
...
def _build_slice_force_operator(
    *,
    ...
    for cross_section in cross_sections:
        line_weights = np.array(cross_section.point_weights, dtype=np.float64, copy=True)
...
def _build_cross_section_extrapolation_weights(
    *,
    ...
    extrapolation_weights = np.zeros(cross_section.point_weights.shape, dtype=np.float64)
