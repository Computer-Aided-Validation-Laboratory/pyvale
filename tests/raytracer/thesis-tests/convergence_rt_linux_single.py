...
# THIS RENDERS A SINGLE IMAGE for a specified element
...
def conv_test_rt(test_case: TestCase,
    ...
    for idx, (name, element) in enumerate(iter_elements()):
        if element_idx is not None and idx != element_idx:
            print(f"Idx: {idx}")
            continue   # skip all elements except the one we want
...
conv_test_rt(TestCase.TANK, Resolution.HIGH, 2, None, 3)